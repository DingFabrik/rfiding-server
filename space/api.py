    ...
    def get(self, request, format=None):
        ...
        if request.GET.get('secret', None) is not None:
            ...
            if new_state == is_open:
                return Response({"open": is_open, "changed_at": current_state.updated}, status=status.HTTP_200_OK)
            state = SpaceState.objects.create(is_open=new_state)
            state.save()
            return Response({"open": state.is_open}, status=status.HTTP_200_OK)
        return Response({"open": current_state.is_open, "changed_at": current_state.updated}, status=status.HTTP_200_OK)
