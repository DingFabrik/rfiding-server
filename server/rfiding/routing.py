...
websocket_urlpatterns = [
    ...
    re_path(
        r"ws/machines/(?P<mac_address>([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2}))/state",
        MachineStateConsumer.as_asgi(),
    ),
    re_path(
        r"ws/machines/(?P<mac_address>([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2}))/logs",
        MachineLogConsumer.as_asgi(),
    ),
]
