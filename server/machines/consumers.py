...
class MachineStateConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        machine = await Machine.objects.aget(mac_address__iexact=mac_address, is_active=True)
    ...
    async def state_update(self, state):
        html = render_to_string("machine_status_partial.html", {
            "status": state["state"],
        })
...
class MachineLogConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def state_update(self, log):
        html = render_to_string("machine_log_partial.html", {
