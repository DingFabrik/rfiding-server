...
class CheckMachineTests(APITestCase):
    ...
    def test_qualification_if_always_alow_closed(self):
        ...
        person = Person.objects.create(name='test', email='test@example.com')
        person.save()
        qualification = Qualification.objects.create(machine=machine, person=person, permission_level='always')
        qualification.save()
        token = Token.objects.create(serial='456', person=person)
        token.save()
        state = SpaceState.objects.create(is_open=False)
    ...
    def test_active_time(self):
        ...
        machine = Machine.objects.create(mac_address='aa:bb:cc:dd:ee:ff', hostname='test', name='test')
        machine.save()
        machinetime = MachineTimes.objects.create(machine=machine, weekdays="1,3", start_time='00:00', end_time='22:00')
        machinetime.save()
        person = Person.objects.create(name='test', email='test@example.com')
        person.save()
        qualification = Qualification.objects.create(machine=machine, person=person, permission_level='always')
    ...
    def test_inactive_time(self):
        ...
        machine = Machine.objects.create(mac_address='aa:bb:cc:dd:ee:ff', hostname='test', name='test')
        machine.save()
        machinetime = MachineTimes.objects.create(machine=machine, weekdays="0,1,3", start_time='00:00', end_time='22:00')
        machinetime.save()
        response = self.client.get(CheckMachineTests.url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data['access'], 0)
    ...
    def test_inactive_day(self):
        ...
        machine = Machine.objects.create(mac_address='aa:bb:cc:dd:ee:ff', hostname='test', name='test')
        machine.save()
        machinetime = MachineTimes.objects.create(machine=machine, weekdays="0,3", start_time='00:00', end_time='22:00')
        machinetime.save()
        response = self.client.get(CheckMachineTests.url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data['access'], 0)