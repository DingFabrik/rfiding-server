...
class ConfigureMachineTimeForm(forms.ModelForm):
    weekdays = forms.MultipleChoiceField(choices=DAY_CHOICES, widget=forms.CheckboxSelectMultiple)

    class Meta:
        model = MachineTimes
        fields = ['weekdays', 'start_time', 'end_time']

MachineTimeFormset = forms.modelformset_factory(MachineTimes, extra=3, max_num=7, form=ConfigureMachineTimeForm)